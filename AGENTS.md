...
- Work on NexaStock only under `inventory/`. The root `app/` and `sql/` are preserved CRM comparison material; never import their destructive SQL into an inventory database.
